...
def Rotate(deg=20):
    ...
    def _rotate(img):
        ...
        :returns: H x W x C numpy array
        """
        # Generate a random integer degree in the range (-deg, deg)
        rotation_deg = np.random.randint(-deg, deg + 1)

        # Rotate the image and fill the missing pixels with black
        rotated_img = rotate(img, rotation_deg, reshape=False, mode='constant', cval=0)
...
def Grayscale():
    ...
    def _grayscale(img):
        ...
        grayscale_values = np.mean(img, axis=-1).astype(np.uint8)

        # Convert it back to a 3-channel image
        grayscale_img = np.stack([grayscale_values] * 3, axis=-1)
...
def main(args):
    ...
    augmentations = [Rotate()] # rotation only 
    # augmentations = [Grayscale()] #grascale only 
    # augmentations = [Rotate(), Grayscale()] #both rotation and grayscale
